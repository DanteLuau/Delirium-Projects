...
function Window:_Focus()
    if self._destroyed then return end
    self.MainFrame.ZIndex = _nextFocusZ()
end
...
function Button.New(parent: Instance, config: table)
    ...
    local function playSuccess()
        -- Component may have been destroyed while loading; never touch
        -- destroyed instances or flip state on a dead component.
...
function Dropdown.New(parent: Instance, config: table)
    ...
    -- Stored so api:Destroy() can disconnect it (unmanaged connections leak).
    local canvasConn = UIList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            ...
            table.insert(optionConns, Btn.Activated:Connect(function()
                ...
                if isMulti then
                    ...
                    task.delay(TweenHelper.FastInfo.Time, function()
                        if _destroyed or not OptionScroll.Parent then return end
                        if not isOpen then
